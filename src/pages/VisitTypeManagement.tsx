...
import { Plus, Loader2, AlertCircle, Edit2, Trash2, X, ClipboardList } from 'lucide-react';
...
export default function VisitTypeManagement() {
  const [data, setData] = useState<VisitType[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const result = await api.getVisitTypes(token);
      setData(result);
      setError('');
    } catch (err: any) {
      setError(err.message || 'Failed to fetch visit types');
    } finally {
      setLoading(false);
    }
  }, [navigate]);
  ...
  const openEdit = (item: VisitType) => { setEditing(item); setFormValue(item.visitType || ''); setFormError(''); setIsFormOpen(true); };
  const closeForm = () => { setIsFormOpen(false); setEditing(null); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formValue.trim()) { setFormError('Visit type is required'); return; }
    const token = localStorage.getItem('jwt');
    if (!token) return;
    try {
      ...
      setFormError('');
      if (editing) {
        await api.updateVisitType(token, editing.documentId || editing.id, { visitType: formValue });
      } else {
        await api.createVisitType(token, { visitType: formValue });
      }
      await fetchData();
      closeForm();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string | number) => {
    ...
    try {
      setMutationLoading(true);
      await api.deleteVisitType(token, id);
      await fetchData();
    } catch (err: any) {
      setError(err.message || 'Failed to delete visit type');
    } finally {
      setMutationLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-bold text-zinc-900 tracking-tight">Visit Type Management</h1>
          <p className="text-zinc-500 mt-2">Manage accreditation visit types.</p>
        </div>
      ...
      {loading && data.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[32px] border border-zinc-100 shadow-sm">
          ...
          <p className="text-zinc-500 font-medium">Loading visit types...</p>
        </div>
      ) : data.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[32px] border border-zinc-100 shadow-sm">
          ...
          <p className="text-zinc-500 font-medium text-lg">No visit types found</p>
          <p className="text-zinc-400 text-sm mt-1">Add your first visit type to get started.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-widest">Visit Type</th>
              ...
              <tbody className="divide-y divide-zinc-100">
                {data.map((item) => (
                  <tr key={item.id} className="hover:bg-zinc-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm font-bold text-zinc-900">{item.visitType || 'N/A'}</td>
          ...
          <div className="w-full max-w-md bg-white rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl font-bold text-zinc-900">{editing ? 'Edit Visit Type' : 'Add Visit Type'}</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2">Visit Type <span className="text-red-500">*</span></label>
                  <input type="text" value={formValue} onChange={(e) => setFormValue(e.target.value)} placeholder="e.g. Preliminary Visit" className="w-full px-5 py-4 bg-zinc-50 border border-zinc-200 rounded-2xl text-zinc-900 font-medium focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all outline-none" disabled={mutationLoading} />
                </div>
