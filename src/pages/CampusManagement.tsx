...
import { Plus, Loader2, AlertCircle, Edit2, Trash2, X, Building2 } from 'lucide-react';
...
export default function CampusManagement() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const result = await api.getCampuses(token);
      setData(result);
      setError('');
    } catch (err: any) {
  ...
  const openEdit = (item: Campus) => { setEditing(item); setFormValue(item.campusDesc || ''); setFormError(''); setIsFormOpen(true); };
  const closeForm = () => { setIsFormOpen(false); setEditing(null); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formValue.trim()) { setFormError('Campus description is required'); return; }
    const token = localStorage.getItem('jwt');
    if (!token) return;
    try {
      ...
      setFormError('');
      if (editing) {
        await api.updateCampus(token, editing.documentId || editing.id, { campusDesc: formValue });
      } else {
        await api.createCampus(token, { campusDesc: formValue });
      }
      await fetchData();
      closeForm();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string | number) => {
    ...
    try {
      setMutationLoading(true);
      await api.deleteCampus(token, id);
      await fetchData();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-bold text-zinc-900 tracking-tight">Campus Management</h1>
          <p className="text-zinc-500 mt-2">Manage campus locations.</p>
        </div>
      ...
      {loading && data.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[32px] border border-zinc-100 shadow-sm">
          ...
          <p className="text-zinc-500 font-medium">Loading campuses...</p>
        </div>
      ) : data.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[32px] border border-zinc-100 shadow-sm">
          ...
          <p className="text-zinc-500 font-medium text-lg">No campuses found</p>
          <p className="text-zinc-400 text-sm mt-1">Add your first campus to get started.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-widest">Campus Description</th>
              ...
              <tbody className="divide-y divide-zinc-100">
                {data.map((item) => (
                  <tr key={item.id} className="hover:bg-zinc-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm font-bold text-zinc-900">{item.campusDesc || 'N/A'}</td>
          ...
          <div className="w-full max-w-md bg-white rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl font-bold text-zinc-900">{editing ? 'Edit Campus' : 'Add Campus'}</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2">Campus Description <span className="text-red-500">*</span></label>
                  <input type="text" value={formValue} onChange={(e) => setFormValue(e.target.value)} placeholder="e.g. Main Campus" className="w-full px-5 py-4 bg-zinc-50 border border-zinc-200 rounded-2xl text-zinc-900 font-medium focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all outline-none" disabled={mutationLoading} />
                </div>
